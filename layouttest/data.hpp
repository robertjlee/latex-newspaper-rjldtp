...
private:
  ...
  double length_;
public:
  ArticleOption(int numCols, double length);
  ArticleOption(const ArticleOption &o);
  ArticleOption operator =(const ArticleOption &o);
  ~ArticleOption();
  int numCols() const;
  double length() const;
  double area(double colWidth) const;
  double layoutWidth(double colWidth) const;
...
public:
  Article(int artId);
  ~Article();
  void addOption(int numCols, double length);
...
public:
  ...
  void addArticle(int artId, int headingLength, double colInches);
